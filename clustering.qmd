...
There is likely a lot of seaonality and trends in the data, so we will apply ARIMA to address these things prior to any modeling.

```{r}
start_date <- c(2012, 7, 1)

end_date <- c(2024, 9, 23)

vars_number <- data_clean |>
...
s <- Sys.time()
data_smooth <- HoltWinters(data_ts, seasonal = "additive")
e <- Sys.time()
difftime(e, s)
...
ggplot(data_clean) +
  geom_line(aes(datetime_utc, number_of_persons_injured))
...
```{r}
write_rds(data_clean, "./data/clusters.rds")
```